...
window.onload = function() {
    ...
    const generateCode = () => {
        const funcCode = () => {
            const replayFunctionCode = replayCompressedText.toString();
            const arr = replayFunctionCode.split('\n');
            const fixedIndentation = arr.map((line) => {
                if (line.startsWith('    ')) {
                    return line.slice(4);
                }
